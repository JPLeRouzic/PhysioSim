...
public abstract class ASTFunction extends ASTNode {
	...
	/**
	 * Given a function name, will return the {@link ASTFunctionType} for that name, or <code>null</code> if the
	 *  function type with that name does not exist.
	 * @param fName A lower-case <code>String</code> corresponding to a function supported by this library. <br/>
	 *   E.g., 'sin', 'cos'.
	...
	/**
	 * Represents the factorial element in MathML. Evaluation is limited to integers; the gamma
	 *  function is not supported. If the single argument is a double, it is first converted to an integer.
	 *  
	 * @author radams
	 *
	 */
	public static class ASTFactorial extends ASTFunction {
		...
		/**
		 * CAlculates the factorial of an integer representation of its child element.
		 * @return the factorial of the first child element, or 1 if first child element evaluates to < 1.
		 */
		ASTNumber evaluate0(IEvaluationContext con) {
	 ...
	 * @param base A positive <code>int</code>
	 * @return A logElement with the set base.
	 * @throws IllegalArgumentException if <code>base</code>  < 1
	 */
	...
	/**
	 * Represents a miscellaneous or unknown function; will evaluate to {@link ASTNumber.NULL_NUMBER}
	 * @author radams
	 *
	 */
	...
	public static class ASTRoot extends ASTFunction {
		...
		ASTNumber evaluate0(IEvaluationContext con) {
			double answer = Double.NaN;
				...
				if (isSqrt()) {
					...
					answer = Math.pow(getRightChild().evaluate(con).getValue(), (double) 1 / number.getValue());
				}
		...
		public boolean isSqrt() {
			return getNumChildren()==1 || getNumChildren()==2 && firstChild().getName().equals("2");
		}
		
		 boolean isCbrt(IEvaluationContext con) {
			return getNumChildren()==2 && getFirstChildEvaluatedValue(con) - 3 < 0.00001;
		}
