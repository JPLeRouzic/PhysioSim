 ...
 */
 class Tokenizer {
	 final static String NAME_STR = "\\b[_A-Za-z]\\w*\\b"; // use word boundaries: won't match "e10" from "1e10", as that's a number
	 final static String NUMBER_STR  = "\\b[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?\\b"; // use word boundaries: won't match "6" from "k6", as 
	 ...
	 final static Pattern [] patterns = new Pattern[] {NAME,NUMBER,LPAR, RPAR, ARG_SEP, PLUS, MINUS, TIMES,DIVIDE,POW,EQ,NOT,NEQ,LEQ,LT,GEQ,GT,AND,OR};
	...
	TokenStream tokenize(String input) throws  ParseException{
		...
		for (RawToken rawtok: rawTokens){
			...
			if(rawtok.isMatched()==false){
				throw new ParseException("[" +rawtok.getRawToken() + "] could not be parsed at position: " + indx);
			}
			indx++;
		}
