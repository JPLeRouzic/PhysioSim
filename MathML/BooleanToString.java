...
import MathML.ASTRelational.ASTRelationalType;

 class BooleanToString {
	
	/**
	 * Takes a logical or relational node of arbitrary complexity and returns a C-style representation
	 * @param logicalOrRelational An {@link ASTNode} of type {@link ASTRelationalType} or {@link ASTLogicalType}
	 * @return A <code>String </code> representation
	 * @throws IllegalArgumentException if node is incorrect type.
	 */
			...
			for (ASTNode ineq: bottom.getChildren()){
				if(ineq.getUserData()!=null){
					...
					if (ineq != bottom.getRightChild()){
						  sb.append(" " + bottom.getString()+ " "); // ineq operator
					  }
				} else {
				createInequality(sb, ineq);
					if (ineq != bottom.getRightChild()){
						sb.append(" " +bottom.getString() + " "); // logical operator
					}
				}
			}
