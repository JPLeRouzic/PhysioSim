...
import MathML.ASTNode.ASTType;

/**
 * Converts an AST to a string representation of a formula. Currently handles functions, 
 * arithmetic and numerical construct, and simple inequalities. <p/>
 * The strings generated are not automatically in correct C syntax, but are generated based on
 * MathML terminology. For example, the natural logarithm, ln, is translated into the string 'ln',
 *  not 'log' as it is in C.
 */
public class FormulaFormatter {

	/**
	 * 
	 * @param node A non-null node to extract the formula
	 *  for
	 * @return The formula as a String
	 * @throws IllegalArgumentException if <code>node</code> is null.
	 * @link http://sourceforge.net/apps/mediawiki/jmathml/index.php?title=Import_of_C-style_Math
	 */
	...
	boolean FormulaFormatter_isFunction(final ASTNode node) {
		return node.isFunction() || node.isLogical() || (node.isSymbol() && ((ASTSymbol)node).isSymbolFunction());
	}
	...
	/**
	 * Formats the given ASTNode as a token and appends the result to
	...
	/**
	 * Formats the given ASTNode as a function name and appends the
	...
	/**
	 * Formats the given ASTNode as an operator and appends the result
