...
/**
 * Base class for all ASTNodes. All MathML elements are linked as an Abstract Syntax Tree
 * For all these elements, equality is based on instance. For example, if an expression makes 
 *  multiple references to a &lt;ci&gt;x&lt;/ci&gt; element, these should be added to the ASTNode
 *  tree as separate objects.
 *  <p>
 ...
 *   
 *  </p>
 *  For expressions containing variables, these variables need to be resolved at evaluation time by
 *   supplying the evaluation function with an {@link IEvaluationContext}.
 * @author radams
 *
 */
public abstract class ASTNode {
	...
	/**
	 * Gets the level within the document of this node. The root element is at level 0,
	 *  its children at level 1, etc.,
	...
	/**
	 * Gets the name of this node which may be a descriptor or actual content depending on the subclass.
	...
	public  final  String getName(){
	 ...
	 /**
	  * Convenience method to return the names of all Ci element names
	  *    in the tree descending from this node.<br/>
	  *    Since ASTCi node equality is based on object identity, this method may return   multiple objects referring to 
	  *     the same variable.
	  *   
	  * @return  A possibly empty but non-null set of variable names in this node and its descendants.
	  */
	 ...
	 /**
	  * Convenience method to return the  names of all unique Ci element names
	  *    in the tree descending from this node.<br/>
	  *   
	  * @return  A possibly empty but non-null set of variable names in this node and its descendants.
	  */
	 ...
	 * check if this is semantically correct, addition may produce the incorrect number
	 * of child nodes for a given type.
	 * @param node An {@link ASTNode}, not null.
	 * @return <code>true</code> if added, <code>false </code>otherwise.
	 * @throws IllegalArgumentException if argument <code>node</code> is null
	 */
	 ...
	 * @param node
	 * @return
	 * @throes IndexOutOfBoundsException - if the index is out of range (index < 0 || index > size()).
	 */
	...
	/**
	 * Operation to remove a non-null child node . This operation does not
	 * check if this is semantically correct, removal may produce the incorrect number
	 * of child nodes for a given type.<br/>
	 *  The removed node will have a null parent, and its level
	 * will be set to 0.
	 * @param node, not null
	 ...
	 * @return The parent <code>ASTNode</code>, or <code>null</code> if not parented
	 */
	...
	/**
	 * Convenience method to get first child. This is equivalent to:<p/> 
	 * <code> node.getChildAtIndex(0);</code>
	 * @return the first child {@link ASTNode};
	 * @throws IllegalStateException if has no child.
	 */
	 ...
	 * @throws IllegalArgumentException if index is outside the range of  the child list.
	 */
	public final ASTNode getChildAtIndex (int index){
		if (indexOK(index))
		 return children.get(index);
		else throw new IllegalArgumentException("index ouside range");
	}
	...
	public final ASTNode removeChildAtIndex (int index){
		...
		else throw new IllegalArgumentException("index outside range");
	}
	
	/**
	 * This method will replace an old child node with a new child node, if the old child node
	 * is indeed a child of this node. The new node will occupy the same index as the old one did.
	 * @param oldNode A non-null node to remove.
	 * @param newNode  A non-null node to add.
	 * @return <code>true if replacement was successful</code>, <code>false</code> if the node to be changed 
	 *  was not found.
	 ...
	 * @return the zero based index of this node amongst its siblings. If it has no siblings, index = 0.
	...
	/**
	 *  Return <code>true</code> if this node represents a &lt;csymbol&gt; element.
	 * @return
	 */
	 ...
	 * @throws IllegalStateException if not an operator.*/
	 ...
	 * @param v An {@link ASTVisitor}
	 * @return <code>true</code> if descendants of this node should be traversed.
	 */
	...
	/**
	 * Evaluates the ASTNode as an expression. If the expression contains variables,
	 *  the {@link IEvaluationContext} should resolve these variables at runtime.<br/>
	 *  This can be verified by calling canEvaluate(context).</br>
	 *  Subclasses should override the doEvaluate() method to provide an implementation for a node.
	 * @param context A non-null {@link IEvaluationContext}
	 * @return The result of evaluating the expression
	 * @throws IllegalStateException if the ASTNode cannot evaluate the expression
	 */
	 ...
	 * @return The result of evaluating the expression
	 * @throws IllegalStateException if the ASTNode cannot evaluate the expression.
	 * @see evaluate(IEvaluationContext context)
	 */
	...
	/**
	 * Boolean test as  to whether this AST expression can be evaluated to return a number.
	 * This method recursively explores this node's children. If any child node cannot be evaluated because:
	 * <br>
	 * <ul><li>
	 *  An incorrect number of children </li>
	 *  <li>A variable ( a &lt;ci&gt; term ) cannot be resolved. (I.e., its value has not been set into its evaluation context. )</li>
	 *  <li> This node, or a child node, is an unknown function (type ASTFunctionType.MISCELLANEOUS).
	 *  </ul>
	 *  then this method returns <code>false</code>.
	 * @param context A non-null {@link IEvaluationContext}
	 * @return <code>true</code> if this expression can be evaluated, false otherwise.
	 */
	public final boolean canEvaluate(IEvaluationContext context) {
		if ( (isVariable() && !context.hasValueFor(getName())) ||
				 !hasCorrectNumberChildren() ||
				 isFunction() && getType().equals(ASTFunctionType.MISCELLANEOUS)
	...
	/**
	 * Subclasses can optionally extend this to provide further tests for whether a node can be evaluated.
	 * By default this method returns <code>true</code>.
	 * @param context An {@link IEvaluationContext}
	 * @return A <code>boolean</code>
	 */
	...
	private boolean canEvaluateChildren(List<ASTNode> children, IEvaluationContext context) {
	 ...
	 * @return the r-h child ASTNode of this node, or <code>null</code> if there are no children.
	 */
	...
	/**
	 * Predicate returning true or false depending on whether this ASTNode has the correct number of children for its type.<br/>
	 * E.g., for a 'plus' node, number of children should be >= 2
	 * For a 'sin' node, the correct number of children is 1.
	 * @return <code>true</code> if node has correct number of children. 
	 * @see <a href="http://www.w3.org/TR/MathML2/chapter4.html"> MathML spec </a>
	 */
	 ...
	 * @return <code>true</code> if this node is a logical operator (and, or, xor, not). 
	 */
	...
	/**
	 * Gets the precedence of this node. This is typically only used for function and operator nodes
	 * @return an integer of the node's precedence in expressions. The default is 6.
	 */
	...
	/**
	 * Gets this node as an ASTNumber, if this node's isNumber() returns <code>true</code>
	 * @return 
	 */
	...
	/**
	 * Gets this node as an ASTFunction, if this node's isFunction() returns <code>true</code>
