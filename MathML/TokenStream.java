...
class TokenStream implements Iterable<Token> {
	...
	public TokenStream trimExtraneousParentheses(){
		while(startsWith(new Token("(") )&& endsWith(new Token(")")) && allOneElement()){
			TokenStream ts = new TokenStream(tokens.subList(1, tokens.size()-1));
		...
		class TokenIterator implements Iterator<Token> {
			...
			Token previous(){
				if(currIndx<=0){
					return null;
				}else return tokens.get(currIndx-1);
			}
			...
			/*
			 * Gets next token without advancing the pointer or null there is no next token
			 */
			Token peek(){
				if(hasNext()){
