...
public class ASTRelational extends ASTNode {
	 ...
	 * <li> An expression with a single child returns <code>ASTNumber.FALSE</code> (unless this node
	 *    is of type {@link ASTRelationalType.EQ}), in which case this method returns ASTNumber.TRUE.
	 * <li> An expression with > 1 child  is tested for the relation.
	 * </ul>
	 * @param an {@link IEvaluationContext}
	 * @return {@link ASTNumber.TRUTH} if relation is true, code>ASTNumber.FALSE</code> otherwise.
	 */
	@Override
	...
	private ASTNumber test(IEvaluationContext con, Tester tester) {
		...
		//test 1st and 2nd child, then 2nd & 3rd child etc.,- > if any relation false then return false
	...
	Tester GET= new Tester (){
		public boolean test (ASTNode num1, ASTNode num2, IEvaluationContext con){
			return num1.evaluate(con).getValue() >= num2.evaluate(con).getValue();
		}
	};
	...
	Tester EQ = new Tester (){
		public boolean test (ASTNode num1, ASTNode num2, IEvaluationContext con){
			return Math.abs(num1.evaluate(con).getValue() -  num2.evaluate(con).getValue()) < TOLERANCE;
		}
	};
