...
import MathML.ASTFunction.ASTLog;
import MathML.ASTFunction.ASTRoot;

/**
 * <p>Class for converting ASTNodes of math elements into MathML elements.
 * An  ASTNode passed into one of the methods will be wrapped in a <code>&lt;math&gt;</code>
 *  element.</p>
 *  
 *  <p>This class is stateful. For separate invocations, either use a new object or call clear()
 *  before using again.</p>
 *  
 *  Usage:
 *  
 *  <pre>
...
public class ASTToXMLElementVisitor extends ASTVisitor {
	...
	/**
	 * Returns the generated Element. If this method is called before visit(), this will return
	 * an empty &lt;math&gt; element.
	...
	private boolean parentIsRootToOtherDegree(ASTNumber node) {
		return node.getParentNode()!=null&&node.getParentNode().getType().equals(ASTFunctionType.ROOT) &&
		   !node.getParentNode().isSqrt() && node.getIndex()==0;
	}

	private boolean parentIsLogToOtherBase(ASTNumber node) {
		return node.getParentNode()!=null&&node.getParentNode().getType().equals(ASTFunctionType.LOG) &&
